#!/usr/bin/env python3
"""GMB CORS Proxy - forwards requests to data.etagmb.gov.hk with CORS headers."""
import sys, json, http.server, urllib.request, urllib.error, gzip

GMB_BASE = "https://data.etagmb.gov.hk"

class GMBPxy(http.server.BaseHTTPRequestHandler):
    def do_OPTIONS(self):
        self.cors()
...
if __name__ == "__main__":
    ...
    h = http.server.HTTPServer(("0.0.0.0", p), GMBPxy)
    print(f"GMB CORS Proxy running on http://localhost:{p}")
    h.serve_forever()
