...
│           ▼                  ▼                        │
│  ┌──────────────┐  ┌─────────────────┐               │
│  │ KMB Open API  │  │  GMB Proxy       │              │
│  │ (CORS OK)     │  │ (localhost:8899) │              │
│  └──────┬───────┘  └────────┬────────┘               │
│         │                   │                        │
└─────────┼───────────────────┼─────────────────────────┘
          │                   │
          ▼                   ▼
  data.etabus.gov.hk   data.etagmb.gov.hk
  (KMB)                (綠色專線小巴)
```
...
3. **GMB 到站資料**: 瀏覽器呼叫 `http://localhost:8899/eta/stop/{stopId}` → gmb_proxy.py 轉發至 `https://data.etagmb.gov.hk/eta/stop/{stopId}` → 回傳含 CORS headers 的回應
4. **GMB 路線資料**: 瀏覽器呼叫 `http://localhost:8899/route/NT/{code}` → 同上機制轉發
...
| 分頁 ID | 顯示名稱 | Stop IDs | 路線數 | GMB 路線 |
|---------|---------|----------|--------|---------|
...
- 無需額外 Python 套件（gmb_proxy.py 僅使用標準函式庫）

### 2.3 開發用啟動（含 opencode MCP）
...
**注意**: GitHub Pages 上 GMB 資料無法顯示（因無法跑 proxy）。需在本機執行 proxy 才能取得 GMB ETAs。

### 2.5 故障排除

| 問題 | 原因 | 解決方法 |
|------|------|---------|
| GMB 路線顯示「暫未服務」 | GMB Proxy 未啟動 | 執行 `python gmb_proxy.py` |
| Proxy 埠被佔用 | 其他程式使用相同埠 | `python gmb_proxy.py 8900`，並修改 index.html 中 `GMB_API` |
...
GET /route/NT/{routeCode}               — GMB 路線資料
```
